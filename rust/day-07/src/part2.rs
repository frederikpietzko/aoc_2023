...
impl Hand {
    ...
    fn hand_type(&self) -> HandType {
        ...
        let jokers = *map.get(&Card::Joker).unwrap_or(&0usize);

        match (map.len(), jokers) {
            (1, _) => HandType::OAK5,
            (2, 0) => {
            ...
            (3, x) => {
                let counts: (usize, usize, usize) = map
                    ...
                    .expect("Invalid hand");
                match (counts, x) {
                    ((1, 1, 3), 3) => HandType::OAK4,
                    ((1, 1, 3), 1) => HandType::OAK4,
                    ((1, 1, 3), 0) => HandType::OAK3,
                    ((1, 2, 2), 0) => HandType::TP,
                    ((1, 2, 2), 1) => HandType::FH,
                    ((1, 2, 2), 2) => HandType::OAK4,
                    _ => panic!("Invalid hand: {:?}", self.cards),
                }
            }
            (4, 0) => HandType::OP,
